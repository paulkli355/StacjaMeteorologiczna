...
namespace MeteoWF
{
    public partial class Today : UserControl
    {
        ...
        {
            var temp = context.Pomiaries.Where(x => x.DataCzas.Day == (DateTime.Now).Day - 7).Average(x => x.Temperatura);
            double srtemp = Math.Round((double)temp, 1);
            TempWar.Text = string.Format(srtemp.ToString() + " °C");

            var humid = context.Pomiaries.Where(x => x.DataCzas.Day == (DateTime.Now).Day - 7).Average(x => x.Wilgotnosc);
            double srhumid = Math.Round((double)humid, 1);
            HumidWar.Text = string.Format(srhumid.ToString() + " %");

            var srednia1 = context.Pomiaries.Where(x => x.DataCzas.Day == (DateTime.Now).Day - 7).Average(x => x.PM1);
            double sr1 = Math.Round((double)srednia1, 1);
            PM1text.Text = string.Format(sr1.ToString() + " µg/m3");

            var srednia25 = context.Pomiaries.Where(x => x.DataCzas.Day == (DateTime.Now).Day - 7).Average(x => x.PM25);
            double sr25 = Math.Round((double)srednia25, 1);
            PM25text.Text = string.Format(sr25.ToString() + " µg/m3");

            var srednia10 = context.Pomiaries.Where(x => x.DataCzas.Day == (DateTime.Now).Day - 7).Average(x => x.PM10);
